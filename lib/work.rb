...
#   -s, --svg           If not based on another work, create a new svg (default)
#   -p, --psd           If not based on another work, create a new psd
#
# == Author
#    Rhea Myers <rhea@myers.studio>
#
# == Copyright
#    Copyright (C) 2008 Rhea Myers. Licensed under the GNU GPL 3 or later.
...
class Work
  ...
  def process_arguments
    ...
    if @source_work_name == nil
      if File.extname(@destination_work_name) == '.svg'
        @source_work_name = "#{@resource_dir}/template.svg"
      elsif File.extname(@destination_work_name) == '.psd'
        @source_work_name = "#{@resource_dir}/template.psd"
      else
        puts "Unknown file format for work."
      end
    end
  end
